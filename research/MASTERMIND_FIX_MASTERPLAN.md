...
- **2026-07-25 (Fable, operator-ordered): LLM COST RULING — flagship burn cut + honest attribution.** Root-caused the admin AI-Cost "oauth" burn ($194.64 shadow-USD/30d, flagship $59.12 = 30%): (1) role="deep" was a catch-all Opus bucket — strategist, gate/risk officers, sentinel-per-name loop, macro_risk (intraday derisk/watch lanes), CIO, and paper digests ALL billed book=flagship seat="deep" via `_ROLE_BOOK`; (2) the research desk's armed session (the single biggest LLM leg) was **invisible** — `book="flagship"` invoked the caller-records contract and nobody recorded; (3) every budget control was disarmed (`MASTERMIND_NIGHTLY_USD_CAP` unset; `brain.yml monthly_cost_cap_usd:75` decorative). SHIPPED: `research_max_turns` 16→8; `FLAGSHIP_PM_MAX_TURNS` 30→12; `MASTERMIND_NIGHTLY_USD_CAP=6.0` armed (sized above a normal post-ruling flagship day ~$4-5 so it clips only blowouts; shrink-only, charter-compliant); re-tiered to sonnet: sentinel (subtract-only + calibration-shrunk ⇒ authority unchanged), macro_risk narrative (deterministic state holds the teeth), paper digest/redigest, and all three format-only JSON retries — strategist/gate/risk-officer MAIN calls + PM + research desk stay opus; seat/record_book attribution plumbing through cli_bridge+client (named seats: research_desk, sentinel, strategist, gate_officer, risk_officer, macro_risk→system, cio→system, paper_digest→system); research desk now RECORDS its session (seat=research_desk, per-key) and honours the tripwire; bot/etf.py got the missing over_budget gate (every sibling had it). Tests: 4 call_model stubs modernised to the new signature; targeted suites green; incident replays 19/19 — `test_composed_stack_flag_on_disagreeing_tape` was red in the PRODUCTION checkout only: a pre-existing hermeticity leak (live vendor/macro regime latest.json diluted the tape's intended D 0.5433 → 0.273/BALANCED; proven clean-worktree-green with the full change-set layered in; cap on/off invariant). FIXED with Fable sign-off (charter replay-edit review = this ruling): battery-local READ-side isolation conftest (regime_frame + posture artifacts → empty tmp, restoring the authored fresh-worktree baseline; parent conftest already isolates the write side) + the tape's own recorded regime injected in-test — assertion untouched. Supersedes the earlier follow-up chip (battery-hermeticity), which is now stale. Perf context (CIO 2026-W29): flagship worst book (−4.2% abs) while heavyweight (1 call/day, flagship-subset mandate) is flat to SPY — the expensive layer wasn't paying. Expected: flagship visible/day ~$6-12 → ~$4-5 (now INCLUDING the previously-hidden research leg), deep-seat singles ~5× cheaper at sonnet.

- **2026-07-19 (Fable, W8 — THE ENTRY ERA): FLAGSHIP V2 DECISION CORE SHIPPED** (branch `fable/flagship-v2`; design `research/FLAGSHIP_V2_DECISION_CORE.md`; operator-ordered binding-at-birth, superseding the ENTRY_DISCIPLINE advisory clock — its W0 annotator absorbed). The buy triad: Quality × ENTRY (`portfolio/entry_engine.py` — range/off-low/rollover/late-leg/chase + signal_gate T-tiers + stage/bottom sensors + Prophet plan geometry) × CONTEXT (`portfolio/context_gate.py` — sector_pulse heat, theme_context leadership/breaking, NW contagion lobe, momentum-unwind, risk-regime tightening; tighten-only per the vol_regime asymmetry). PROPHET feed (`portfolio/prophet_feed.py`) = additive candidate source + plan-geometry chase guard. Leadership sleeve second pass (SMH/XLK/MTUM/QUAL class → freed budget to cash + park). Core-path watchlist re-review (was judgment-gated = dark) + trigger promotion + rotation-lane enrollment (base_turn positive path, `MASTERMIND_ROTATION_IN` default `watch`). Research gate reframed to the quality axis w/ injected entry/context/prophet evidence + entry_agreement DE-ESCALATION-only. Unflagged repairs: trend-lens near-high-as-confirmation (D1) + `_MIN_VOTES_FOR_UP=5` thin-vote floor (D3 — the RF 3-vote confluence-1.0 class). Defaults armed: `MASTERMIND_ENTRY_GATE=1`, `MASTERMIND_PROPHET_FEED=1`, `MASTERMIND_NW_CONTEXT=1` (W-NW.1 arming condition matured on its 07-19 come-back), `MASTERMIND_NW_DECISION=shrink` (garbled values now fail to `off`, never escalate). ACCEPTANCE: 17-test replay battery on real 2026-07-18 fixtures — AAPL(extended@range-top+ctx-blocked) ANET(knife+theme-broken) SMH(theme-broken+contagion) XLK(ctx-blocked downstream) MTUM(unwind-blocked) QUAL/V/SCHW/RF(extended/rollover/thin-vote) ALL stopped; XLE (the operator's one good buy) = the ONLY triad pass (clean entry + tier-eligible + energy in leadership strength). Full suite: ZERO new failures (the 18+1 catalogued/pre-existing only, each proven on a master baseline under the same live vendor); incident replays otherwise green; sparse set +`data/stage_analysis/context` (self-migrates). Kill criteria stand per ENTRY_DISCIPLINE §5 (chase-vs-clean forward entry-drawdown over ≥40 graded, per-gate demotion to display).

- **2026-07-05 (Sonnet W-NW.1): NW context bridge dark-shipped** (branch `fable/nw-context-bridge`): `brain/neural_web_context.py` (single reader — fail-soft, schema/freshness-validated, process cache, `context()`/`candidate()`/`market_plane()`/`seat_prompt_block()`/`audit_row()`/`nw_prompts_enabled()`); `brain/market_view.py` (`neural_web` appended to PLANE_ORDER + `_adapt_neural_web()` adapter always `validated=False`, `neural_web_out` kwarg threaded through `build()` and `view()`); `bot/phase2.py` (flag-independent NW audit `perception` runlog row + `_nw_plane` assembled when flag ON + `neural_web_out` passed into `_mv_mod.build()` + optional `nw_context` key in shadow-inputs rows, absent-safe); `brain/pm_conviction.py` (lazy NW text block ≤1200 chars, flag-gated, after E2.5 posture, no payload key, cortex structurally excluded); `brain/strategist.py` (`neural_web_context` key in payload, flag-gated, `{}` when OFF/absent); test suite: `tests/test_neural_web_context.py` (fail-soft battery + cortex sentinel + market_plane shape + audit_row statuses + market_view acceptance + pm_conviction flag-off/on) + `tests/fixtures/mastermind_context.json` (v1 skeleton with CORTEX_SENTINEL_XYZ). Pre-existing failures on master: 19 (unchanged). **DARK SHIP: `MASTERMIND_NW_CONTEXT` defaults OFF.** Arming condition: ≥5 consecutive builds with `nw_context status=present` in the runlog → operator may set `MASTERMIND_NW_CONTEXT=1` without further Fable review (this ruling IS the review for the prompt-text-only promotion). Come-back 2026-07-19.

- **2026-07-03 (takeover, Fable): W6 SHIPPED + merged + DEPLOYED** (fdbb435, PID 44655): Heavyweight re-mandated firm best-ideas (union of published books, one-per-cluster, mirror as <4-fundable fallback; A/B shadow measures cluster-overlap daily — registry hw-firm-universe-ab, comeback 07-24); brain/book_lifecycle.py (orthogonality matrix + HAC/effective-n honest probation/retire RECOMMENDATIONS via cio+agenda; Self-Directed constitutionally exempt); Self-Directed = the OFFICIAL YARDSTICK (publishes into the firm report at the daily mark sweep, excluded from clamp math — integrator caught + fixed the dead publish() seam); CN edge-led funnel ARMED (CHINA_FUNNEL_PROFILE=edge-led, dashboard-validated, registry cn-funnel-edge-led comeback 07-31); regional bogeys wired. Registry: 14 experiments, all schema-valid. **THE BUILD PROGRAM IS COMPLETE — W7 is a clock**: come-backs 07-17 (posture arming E3.3 + judgment promotion), 07-24 (heavyweight A/B), 07-31 (CN funnel + ablations). Tonight's 22:40 build is the first FULL-stack run.
- **2026-07-03 (takeover, Fable): W-E.2 SHIPPED + merged + DEPLOYED** (b65797f, PID 9067): the posture decider in SHADOW — live first read: **ROTATE_DEFENSIVE, offense 0.43, D 0.558, notch 0.70** — publishing every build. THE SUBSUMPTION delivered (hands-on by Fable after two agent derails): budget() shim w/ evidence-damp bypass, build_def_sleeve(target=) w/ defense-in-depth self-serve, BOTH derisk cutters min-compose the posture notch, phase2 one-shared-read (appetite/floor/provenance). All 22 migration tests are live enforced ship-blockers (xfail convention retired; grep-gate = CI; the test stripper's pass-ordering bug fixed). Registry reseeded (12 items, schema-normalized) after the staging-exclusion loss. E3.3 ARMING remains open: needs the 2-week shadow window (come-back 2026-07-17 in the registry) + Fable review of shadow-vs-live deltas. REMAINING ARC: W6 book mandates → W7 acceptance clock.
- **2026-07-03 (takeover, Fable): W-L SHIPPED + merged + DEPLOYED** (700030b, PID 2624, HEAD==master): ONE marking layer + benchmark ledger (ACCEPTANCE: the defensive basket beats SPY + every Brain on the incident fixtures — the founding fact, now computable in-system); per-seat journals with the conscious duty, pinned-rule falsifiers, and FOUNDING MEMORIES backfilled from the incident (4 seats: crowd-follow / success / label-trust / ignored-plane); weekly improvement agenda (first real AGENDA.md correctly ranks the failed perception gates + un-armed seams as top items); self_tune bounded self-repair (default OFF; Fable boundary review at merge DENIED the entire perception_validation gate-definition block — a tuner that can redefine its own exam labels is unbounded); regime-conditional grading (defensive rotation finally scores well) + allocation attribution + governor (default OFF); 12-item experiment registry with come-back dates; MAINTENANCE.md (the zero-context Opus-only runbook). 1860 passed / 4 catalogued pre-existing / zero new; charter P2/P3/P7/P8 ship-blockers green. REMAINING ARC: posture decider E2/E3 (Fable sign-off), W6 book mandates, W7 acceptance clock.
- **2026-07-03 (takeover, Fable): W-E.0/1 SHIPPED + merged + DEPLOYED** (bde5511, PID 73497): rotation_tensor (incident replay: defensive episode 06-24 @ 98.6th pctile), anticipation battery (notch seams DARK by code), market_view LIVE with 20 planes + label_vs_planes disagreement + deterministic brief (today's brief leads with the radar's CAUTION 75/100 de-risk read); seats' prompts enriched; gate wakes on validated dissent; /market_view page. **ALL THREE E1.4 GATES REPORTED HONEST FAILS** (rotation AUC 0.5426 vs 0.55; crash-risk + label_vs_planes UNCOMPUTABLE without vendored history — H4 named as unlock): everything ships advisory/cold_start, arm seams dark, forward logs accrue from today. P3 held under pressure three-for-three. W-L (Conscious Learning) build launched.
